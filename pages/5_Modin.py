...
st.set_page_config(page_title="MODİN",
...
""")
st.write("""
    Modin, Python'da dağıtılmış veri işleme için açık kaynaklı bir kütüphanedir. Popüler bir veri işleme kütüphanesi 
    olan pandas'ın performansını artırmak için tasarlanmıştır. Modin, kullanıcıların iş akışlarına minimum kod 
    değişikliği ile paralel işleme ve ölçeklenebilir bellek eklemesine olanak tanır.
    
    Modin ve pandas arasındaki temel fark, Modin'in dağıtılmış işleme için tasarlanmış olmasıdır; bu, birden çok 
    çekirdek ve makinede daha büyük veri kümelerini işlemek için ölçeklenebileceği anlamına gelir. Modin ayrıca pandas'a 
    göre daha kullanıcı dostu bir API sunarak kullanıcıların iki kütüphane arasında geçiş yapmasını kolaylaştırır.
...
if qselect[:3] == "5 -":
    ...
    st.write("2 - Tabloları Modin DataFrame olarak değişkene kaydetme")
    st.code("""
        film = pd.read_sql("film", conn)
        inventory = pd.read_sql("inventory", conn)
    """)
    ...
    st.write("3 - Merge işlemleri")
    st.code("""
        merged = pd.merge(customer, rental, how="inner", on="customer_id")
        merged = pd.merge(merged, payment, on="rental_id", how ="inner")
    """)
    merged = pd.merge(customer, rental, how="inner", on="customer_id")
    merged = pd.merge(merged, payment, on="rental_id", how="inner")
    ...
    st.code("""
        ...
        groupped = filtered.groupby(["first_name", "last_name"]).sum()
    """)
    filtered = merged[["first_name", "last_name", "amount"]]
    groupped = filtered.groupby(["first_name", "last_name"]).sum()
    ...
    st.code("""
        ...
        groupped = filtered.groupby(["name", "title"]).count()
        result = groupped.sort_values(by="rental_id", ascending=False, inplace=True)
    """)
    filtered = merged[["name", "title", "rental_id"]]
    groupped = filtered.groupby(["name", "title"]).count()
    groupped.sort_values(by="rental_id", ascending=False, inplace=True)
    ...
    filtered = merged[["title", "rental_id"]]
    result = filtered.groupby(["title"]).agg(total_rentals=("rental_id", "count"))
    sorted_result = result.sort_values(by="total_rentals", ascending=True).head(3)
    st.dataframe(sorted_result)
